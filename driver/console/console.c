...
void DMA2_Stream7_IRQHandler(void)
{
    if (DMA_GetITStatus(DMA2_Stream7, DMA_IT_TCIF7) == SET)
    {
        ...
        xSemaphoreGiveFromISR(write_async_semaphore, &pxHigherPriorityTaskWoken); // 释放信号量，通知发送完成
        portYIELD_FROM_ISR(pxHigherPriorityTaskWoken);  // 如果刚刚唤醒了“更高优先级任务”，立刻切换过去执行
